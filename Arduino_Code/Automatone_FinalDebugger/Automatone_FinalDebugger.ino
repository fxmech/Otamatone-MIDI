...
int lowNoteLimit[10] =  {421, 451, 481, 541, 601, 661, 721, 781, 841, 901};
int midNoteVal[10] =    {435, 465, 510, 570, 630, 690, 750, 810, 870, 920};
int highNoteLimit[10] = {450, 480, 540, 600, 661, 720, 780, 840, 900, 950};
//int scale1[10] = { -9, -8, -5, -4, -2, 0, 1, 3, 4, 7};
int scale1[10] = { 0, 1, 4, 5, 7, 9, 10, 12, 13, 16};
//int scale2[10] = { -8, -7, -5, -3, -2, 0, 2, 4, 5, 7};
int scale2[10] = { 0, 1, 3, 5, 6, 8, 10, 12, 13, 15};
int bend = 0;
int noteVal = 0;
int16_t pitch = 0;
...
void playScale() {
  ...
  pitchPotVal = pitchPotVal / 20;
//Serial.print(pitchPotVal);Serial.print("   ");
  for (int i = 0; i < 10; i++) {
  ...
  mouthValMapped = map(mouthVal, 85, 1023, 55, 127);
  Serial.print("M:");Serial.println(mouthValMapped);
  //MIDI.sendControlChange(16, mouthValMapped, 1);
  delay(20);
  ...
  if (pitchPotVal >= 100) { //Do this if pitch pot is being pressed on
    if (prevPitch != pitch) { //Do this if note changed
      if ((millis() - lastDebounceTime) > debounceDelay) {
        if (noteScale == 1) {
          //MIDI.sendNoteOn(60 + volumePotValMapped + scale1[pitch], 127, 1); //(pitch, 1);
          delay(20);
          //MIDI.sendNoteOff(60 + volumePotValMapped + scale1[prevPitch], 0, 1);
          delay(20);
          Serial.print("1:"); Serial.println(pitch);
        } else if (noteScale == 2) {
          //MIDI.sendNoteOn(60 + volumePotValMapped + scale2[pitch], 127, 1); //(pitch, 1);
          delay(20);
          //MIDI.sendNoteOff(60 + volumePotValMapped + scale2[prevPitch], 0, 1);
          delay(20);
          Serial.print("2:"); Serial.println(pitch);
        }
        prevPitch = pitch;
...
Serial.print(mouthVal);Serial.print("  ");
  //Read Volume Pot
